...
public class GoogleGameServicesPlugin extends Plugin {
    ...
    private void saveSnapshot(final PluginCall call, final SnapshotMetadata snapshotMetadata) {
        waitForClosedAndOpen(snapshotMetadata).addOnCompleteListener(task -> {
            if (!task.isSuccessful()) {
                // Opening the snapshot fails with ApiException 4 (SIGN_IN_REQUIRED) when Play Games
                // sign-in was skipped or is still pending; getResult() would rethrow it and crash
                // the main thread. Reject the JS call instead.
                Log.e(TAG, "Could not open snapshot for saving", task.getException());
                call.reject("Could not open snapshot for saving: " + task.getException());
                return;
            }
